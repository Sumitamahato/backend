...
userRoute.get('/',authentication,authorize(["moderator"]),async(req,res)=>{
    try{
    ...
    try{
        ...
        if(data.length>=1){
            bcrypt.compare(password,data[0].password,async(err,result)=>{
                if(result){
                    const token = jsonwebtoken.sign({user_email : data[0].email},process.env.secret,{expiresIn:'60ms'});
                    const refresh_token = jsonwebtoken.sign({user_email : data[0].email},process.env.refresh,{expiresIn:'180ms'});
